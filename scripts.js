...
function refresh() {
    ...
    if (switchColor) {
        updateColor(darkColor, lightColor, true, true)
    } else {
        updateColor(lightColor, darkColor, false, true)
    }
}

const ratioValidate = (color_1, color_2) => {
    // funcion que filtra los colores que hacen contraste de los que no
...
const updateShadows = () => {
    // funcion que actualiza las sombras segun la lumunosidad del padre
    ...
    try {
        ...
        console.error('error de formato en updateShadows: ' + e)
    }
}
...
function rgbaToHex(rgbaString) {
    ...
    //ojo el alfa no sera usado al final pero igualmente es codigo rgb válido
...
function updateFormatControls(newFormat) {
    ...
    if (newFormat === 'RGB') {
        ...
        inputTextColor_1.value = rgbaToHex(color_1).toUpperCase().slice(0,7); 
        inputTextColor_2.value = rgbaToHex(color_2).toUpperCase().slice(0,7); 
        //garantizamos que se elimine el alfa en ambos
    }
    inputTextColor_1.style.backgroundColor = 'initial'
    inputTextColor_2.style.backgroundColor = 'initial'
}
...
const generateTriangleRamdomColors = () => {
    ...
    for (let i = 0; i < 2; i++) {
        if (i === 0) {
            ...
            color_1 = (Math.floor(Math.random() * (rgbPartition - 16)) + 1) * lowestColor
            color_2 = (Math.floor(Math.random() * (rgbPartition - 16)) + 1) * lowestColor
            color_3 = (Math.floor(Math.random() * (rgbPartition - 16)) + 1) * lowestColor
        }
...
const updateColor = (darkColor, lightColor, switchColors = false, byRandomButton) => {
    //funcion principal que actualiza elementos del dom para que los cambios puedan verse

    //valida si los colores devuelven null
    if (!darkColor || !lightColor) {
        console.error("updateColor: colores indefinidos recibidos:", { darkColor, lightColor });
    ...
    if (formatControl.textContent.trim() === 'RGB') {
        ...
        if (byRandomButton || (inputTextColor_1.length === 0 && inputTextColor_2.length === 0)) {
            inputTextColor_1.value = hexColorDark.slice(0, 7)
            inputTextColor_2.value = hexColorlight.slice(0, 7)
        }
        else if (inputTextColor_1.value.length > 0 && inputTextColor_2.value.length > 0) {
            ...
            inputTextColor_1.value = hexColorDark.slice(0, 7)
            inputTextColor_2.value = hexColorlight.slice(0, 7)
            console.info('error de optimizacion en updateColor, ultimo else')
        }
    }
...
const filterInputText = (inputTextColor_X) => {// filtra los inputs del usuario
    inputTextColor_X.style.backgroundColor = '#ff000022' //resalta el input en rojo para
    //indicarle al usuario que su input no es válido
    
    const value = inputTextColor_X.value.trim().toLowerCase()
    if (value.length < 3) {//optimización un color debe tener al menos 4 caracteres #ABC
        return
    }
    const isHex = /^#?([a-f0-9]{6}|[a-f0-9]{3})$/i // expresion regular para validar hexadecimales
    const isRGB = /^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/; // epresion para validar rgbs (sin alfa)
    const hexTest = isHex.test(value) // devuelve true/false
    const rgbMatch = value.match(isRGB) //devuelve [ rgb(r,g,b), R, G, B ] array de longitud 4
    ...
    if (hexTest) {
        ...
        if (inputTextColor_X.id.endsWith('1')) {
            ...
            inputTextColor_X.style.backgroundColor = 'initial' // restaura el color original del fondo del input
            ...
            inputTextColor_X.style.backgroundColor = 'initial'
        } else {
            return
        }
    } else if(rgbMatch) {
        const colorRGB = rgbMatch[0] // tomamos el color y lo guardamos en una variable
        ...
        if (r >= 0 && r <= 255 && g >= 0 && g <= 255 && b >= 0 && b <= 255) {
            ...
            if (inputTextColor_X.id.endsWith('1')) {
                const color_opossite = colorIndicator_2.style.backgroundColor
                updateColor(colorRGB, color_opossite)
                inputTextColor_X.style.backgroundColor = 'initial'
            } else if (inputTextColor_X.id.endsWith('2')) {
                const color_opossite = colorIndicator_1.style.backgroundColor
                updateColor(color_opossite, colorRGB)
                inputTextColor_X.style.backgroundColor = 'initial'
            } else {
                return
            }
...
inputColor_2.addEventListener('input', () => {
...
inputColor_1.addEventListener('input', () => {
